    ...
    methods
        function obj = bossdevice_oscillation(targetObj, name)
            ...
            if obj.targetObj.isConnected && obj.targetObj.isLoaded
                ...
                obj.phase_plusminus = getparam(obj.targetObj, ['mainmodel/EVD/' obj.name], 'phase_plusminus');
                obj.amplitude_min = getparam(obj.targetObj, ['mainmodel/EVD/' obj.name], 'amplitude_min');
                obj.amplitude_max = getparam(obj.targetObj, ['mainmodel/EVD/' obj.name], 'amplitude_max');
            else
        ...
        function phase_target = get.phase_target(obj)
            phase_target = getparam(obj.targetObj, ['mainmodel/EVD/' obj.name], 'phase_target');
        end
        ...
        function obj = set.phase_target(obj, phi)
            ...
            setparam(obj.targetObj, ['mainmodel/EVD/' obj.name], 'phase_target', newValue);
        end

        
        function phase_plusminus = get.phase_plusminus(obj)
            phase_plusminus = getparam(obj.targetObj, ['mainmodel/EVD/' obj.name], 'phase_plusminus');
        end
        ...
        function obj = set.phase_plusminus(obj, phase_plusminus)
            ...
            setparam(obj.targetObj, ['mainmodel/EVD/' obj.name], 'phase_plusminus', phase_plusminus);
        end

        
        function amplitude_min = get.amplitude_min(obj)
            amplitude_min = getparam(obj.targetObj, ['mainmodel/EVD/' obj.name], 'amplitude_min');
        end
        
        function obj = set.amplitude_min(obj, amplitude_min)
            %METHOD1 Summary of this method goes here
            %   Detailed explanation goes here
            setparam(obj.targetObj, ['mainmodel/EVD/' obj.name], 'amplitude_min', amplitude_min);
        end        


        function amplitude_max = get.amplitude_max(obj)
            amplitude_max = getparam(obj.targetObj, ['mainmodel/EVD/' obj.name], 'amplitude_max');
        end
        
        function obj = set.amplitude_max(obj, amplitude_max)
            %METHOD1 Summary of this method goes here
            %   Detailed explanation goes here
            setparam(obj.targetObj, ['mainmodel/EVD/' obj.name], 'amplitude_max', amplitude_max);
        end

        
        function lpf_fir_coeffs = get.lpf_fir_coeffs(obj)
            lpf_fir_coeffs = getparam(obj.targetObj, ['mainmodel/OSC/' obj.name], 'lpf_fir_coeffs');
        end
        
        function obj = set.lpf_fir_coeffs(obj, coeffs)
            setparam(obj.targetObj, ['mainmodel/OSC/' obj.name], 'lpf_fir_coeffs', coeffs)
        end
        
        
        function bpf_fir_coeffs = get.bpf_fir_coeffs(obj)
            bpf_fir_coeffs = getparam(obj.targetObj, ['mainmodel/OSC/' obj.name], 'bpf_fir_coeffs');
        end
        ...
        function obj = set.bpf_fir_coeffs(obj, coeffs)
            ...
            setparam(obj.targetObj, ['mainmodel/OSC/' obj.name], 'bpf_fir_coeffs', coeffs)
        end        
        
        function offset_samples = get.offset_samples(obj)
            offset_samples = getparam(obj.targetObj, ['mainmodel/OSC/' obj.name], 'ipe_offset_samples');
        end
        
        function obj = set.offset_samples(obj, weights)
            setparam(obj.targetObj, ['mainmodel/OSC/' obj.name], 'ipe_offset_samples', weights)
        end
